...
bool c_base_handle::operator ==(const i_handle_entity* pEnt) const
{
    return get() == pEnt;
}
...
bool c_base_handle::operator <(const i_handle_entity*pEntity) const
{
    unsigned long otherIndex = (pEntity) ? pEntity->get_ref_ehandle().m_Index : INVALID_EHANDLE_INDEX;
    return m_Index < otherIndex;
}

const c_base_handle& c_base_handle::operator=(const i_handle_entity*pEntity)
{
    return set(pEntity);
}

const c_base_handle& c_base_handle::set(const i_handle_entity*pEntity)
{
    if(pEntity) {
